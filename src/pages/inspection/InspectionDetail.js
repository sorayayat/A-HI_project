...
function InspectionDetail() {
  ...
  useEffect(() => {
    if (!isModalOpen) setModifySelf(undefined);
  }, [isModalOpen]);
      ...
      {resume?.data && (
        <div>
          {isModalOpen && (
            <div>
              <ModifyModal
                setModifyIsModalOpen={setIsModalOpen}
          ...
          <div className={[styles.container]}>
            ...
            <div className={styles.selfContent}>
              ...
              <h3>자기소개서</h3>
              <button
                className={styles.modiftBtn1}
                onClick={() =>
                  onClickHandler(resume?.data.SelfIntroduction, 99)
                }
              >
                전체 수정
              </button>
              ...
              <dev className={styles.selfDev}>
                {resume.data.SelfIntroduction.map((self, index) => (
                  <div key={index} className={styles.self}>
                    <button
                      className={styles.modiftBtn2}
                      onClick={() => onClickHandler(self, index)}
                    >
                      수정
                    </button>
